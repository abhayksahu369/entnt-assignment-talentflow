...
const db = new Dexie("TalentflowDatabase");
db.version(1).stores({
    jobs: "++id",
    candidates: "++id,jobId",
...
const defineRoutes = (server) => {
    ...
    server.timing = 750; //latency

    server.get("/jobs", (schema, request) => {
        ...
        const sort = request.queryParams.sort || "order_asc";
    ...
    server.get("/jobs/:id", (schema, request) => {
    ...
    server.post("/jobs", async (schema, request) => {
        if (Math.random() < 0.05) {
        ...
        const newJob = schema.jobs.create({ ...attrs, id, slug, order });
        await saveDbToDexie(server);
        return newJob;
    });
    ...
    server.patch("/jobs/:id", async (schema, request) => {
        ...
        const job = schema.jobs.find(request.params.id);
        const attrs = JSON.parse(request.requestBody);
        const updatedJob = job.update(attrs);
        await saveDbToDexie(server);
        return updatedJob;
    });
    ...
    server.patch("/jobs/:id/reorder", async (schema, request) => {
        ...
        const job = schema.jobs.find(request.params.id);
        const updatedJob = job.update({ order: toOrder });
        await saveDbToDexie(server);
        return updatedJob;
    });
    ...
    server.post("/candidates", async (schema, request) => {
        if (Math.random() < 0.05) {
            return new Response(500, {}, { error: "Server error" });
        }
        const attrs = JSON.parse(request.requestBody);
        ...
        const newCandidate = schema.candidates.create({ ...attrs, id, timeline });
        await saveDbToDexie(server);
        return newCandidate;
    });
    ...
    server.get("/assessments", (schema, request) => {
    ...
    server.put("/assessments/:jobId", async (schema, request) => {
        ...
        if (assessment) {
            updatedAssessment = assessment.update(attrs);
        } else {
            updatedAssessment = schema.assessments.create({ id: nanoid(), jobId, ...attrs });
        }
        await saveDbToDexie(server);
...
export async function makeServer({ environment = "development" } = {}) {
    const hasData = await db.jobs.count() > 0;
    let server;

    if (hasData) {
        const data = await loadDbFromDexie();
        server = createServer({
            environment,
            models: {
                job: Model,
